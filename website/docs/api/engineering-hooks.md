...
- 若插件有设置参数 `IMorPluginXXXOptions`，请将 `options` 的 `interface` 给 `export` 出去，方便可能需要集成的使用方进行引用
...
export interface IMorPluginXXXOptions {
  key: value
}

export default class MorPluginXXX implements Plugin {
  name = 'MorPluginXXX'
  pluginOptions: IMorPluginXXXOptions

  constructor(options?: IMorPluginXXXOptions) {
    this.pluginOptions = {
      ...options
    }
  }
